...
import { loadBranch } from '../../api';

function generateLinks() {
	const links = ['events', 'teams', 'matches', 'players', 'users'];
...
const EditItem = ({ type }) => {
  switch (type) {
    case 'events':
			return <event.components.Admin />
    case 'teams':
			return <team.components.Admin />
		case 'matches':
			return <match.components.Admin />
		default:
...
const Admin = ({ params , dispatch }) => {
	dispatch(loadBranch('admin'));
	return (
	  <div>
	    <p>Edit: {' '}
